...
pub fn draw_list(f: &mut Frame, app: &App, area: Rect) {
    let items: Vec<ListItem> = app
        ...
        .map(|(i, album)| {
            ...
            spans.push(Span::styled(&album.name, theme::title_style()));
...
pub fn draw_detail(f: &mut Frame, app: &App, area: Rect) {
    // Album detail view renders tracks similarly to library
    // but with [AL] tag prefix
    let items: Vec<ListItem> = app
        .tracks
        .iter()
        .enumerate()
        .map(|(i, track)| {
            ...
            spans.push(Span::styled("[AL] ", theme::tag_style()));
